...
export async function installCertInTemporaryKeychain(keychainPath : string, keychainPwd: string, p12CertPath : string, p12Pwd: string) {
    ...
    //create keychain
    var createKeychainCommand : ToolRunner =  tl.createToolRunner(tl.which(iosSecurityToolPath, true));
    ...
    var keychainSettingsCommand : ToolRunner = tl.createToolRunner(tl.which(iosSecurityToolPath, true));
    ...
    var importP12Command : ToolRunner = tl.createToolRunner(tl.which(iosSecurityToolPath, true));
...
export async function findSigningIdentity(keychainPath: string) {
    ...
    var findIdentityCmd : ToolRunner = tl.createToolRunner(tl.which(iosSecurityToolPath, true));
...
export async function getProvisioningProfileUUID(provProfilePath: string) {
    ...
    var getProvProfileDetailsCmd : ToolRunner = tl.createToolRunner(tl.which(iosSecurityToolPath, true));
...
export async function deleteKeychain(keychainPath: string) {
    if (fs.existsSync(keychainPath)) {
        var deleteKeychainCommand : ToolRunner = tl.createToolRunner(tl.which(iosSecurityToolPath, true));
...
export async function unlockKeychain(keychainPath: string, keychainPwd: string) {
    //unlock the keychain
    var unlockCommand : ToolRunner = tl.createToolRunner(tl.which(iosSecurityToolPath, true));
...
export async function getDefaultKeychainPath() {
    ...
    var getKeychainCmd : ToolRunner = tl.createToolRunner(tl.which(iosSecurityToolPath, true));
